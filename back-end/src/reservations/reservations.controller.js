...
function timeIsValid(req, res, next) {
  ...
  if (timePattern.test(reservation_time)) {
    // if valid format
    if (reservation_time < openTime || reservation_time > closeResTime) {
...
function timeResIsValid(req, res, next) {
  ...
  const { reservation_time } = req.body.data;
  const timePattern = /^([01]\d|2[0-3]):([0-5]\d)$/;
  const timePatternMili = /^([01]\d|2[0-3]):([0-5]\d):([0-5]\d)$/
  ////////////
  if (timePattern.test(reservation_time) || timePatternMili.test(reservation_time)) {
    // if valid format
    return next();
  }
  next({ status: 400, message: `Invalid reservation_time.` });
